...
const EditUser = ({ intl }) => {
  ...
  useEffect(() => {
    if (state && Object.keys(state).length > 0) {
      ...
      setValue("companyName", state.item.companyName);
    }
  }, [state]);
  ...
  function handleUpdateSecondaryPassword(e) {
    e.preventDefault();
    const params = {
      id: userData.appUserId,
      password: passwordSecondary,
    };
    UserService.adminChangeSecondaryPasswordUser(params)
      .then(() => {
        toast.success(intl.formatMessage({ id: "reset_password_success" }));
  ...
  const passwordSecondaryChangeHandler = (event) => {
    setPasswordSecondary(event.target.value);
  };
  ...
  const balanceFAC = () => {
    ...
    return (
      <div>
        {balanceAmountFAC ? parseFloat(Number(balanceAmountFAC.balance).toFixed(1)) : ""}{" "}
  ...
  const balanceUSDT = () => {
    ...
    return (
      <div>
        {balanceAmountUSDT ? parseFloat(Number(balanceAmountUSDT.balance).toFixed(1)) : ""}{" "}
  ...
  const balancePoint = () => {
    ...
    return (
      <div>
        {balanceAmountPoint ? parseFloat(Number(balanceAmountPoint.balance).toFixed(1)) : ""}{" "}
        {/* {intl.formatMessage({ id: "FAC" })} */}
      </div>
    );
  };
  const balanceBTC = () => {
    ...
    return (
      <div>
        {balanceAmountBTC ? parseFloat(Number(balanceAmountBTC.balance).toFixed(6)) : ""}{" "}
  ...
  return (
    <Fragment>
      ...
      <Modal
        ...
        toggle={() => setModalSecondary(false)}
        className={`modal-dialog-centered `}
      >
        <ModalHeader toggle={() => setModalSecondary(false)}>
          {intl.formatMessage({ id: "reset_secondary_password_user" })}
        </ModalHeader>
        <ModalBody>
          <FormGroup>
            <Label for="passwordSecondary">
              {intl.formatMessage({ id: "reset_password_secondary" })}
            </Label>
            <Input
              id="passwordSecondary"
              name="passwordSecondary"
              innerRef={register({ required: true })}
              // invalid={errors.passwordSecondary && true}
              placeholder="123456789"
              value={passwordSecondary}
              onChange={passwordSecondaryChangeHandler}
            />
          </FormGroup>
          <FormGroup className="d-flex mb-0">
            <Button.Ripple
              ...
              type="submit"
              onClick={(e) => {
                handleUpdateSecondaryPassword(e);
              }}
            >
              {intl.formatMessage({id:"submit"})}
            </Button.Ripple>
