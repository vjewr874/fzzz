...
function ManageTicketK3({ intl }) {
  ...
  const [betRecordType, setBetRecordType] = useState("K31")
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      maxWidth: "150px",
      cell: () => {
        let currentType = getGameK3Types(intl).find(item => item.value === betRecordType)
        return "K3 - " + currentType?.label || intl.formatMessage({ id: "minutes" }, { val: 1 })
      }
    },
    {
  ...
  return (
    <Fragment>
      ...
      <ModalDetailTicket
        ...
        intl={intl}
        gameName={intl.formatMessage({ id: "game_name" }) + ": K3 - " + getGameK3Types(intl).find(item => item.value === betRecordType).label}
      />
    </Fragment >
  );
}

export default injectIntl(memo(ManageTicketK3));
