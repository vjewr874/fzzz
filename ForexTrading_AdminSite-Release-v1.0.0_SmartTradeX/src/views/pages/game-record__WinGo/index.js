...
function GameRecordPage({ intl }) {
  ...
  const [gameRecordType, setGameRecordType] = useState("WINGO1")
  ...
  const serverSideColumns = [
    ...
    // },
    {
      ...
      maxWidth: "200px",
      cell: () => {
        let currentType = getGameWinGoTypes(intl).find(item => item.value === gameRecordType)
      ...
      cell: (row) => {
        ...
        return (
          <>
            ...
            <div className="valuediv">
              <Badge
                color={sum < 5 ? 'danger' : "success"}
              >{intl.formatMessage({ id: sum < 5 ? "small" : "big" })}</Badge>
            </div>
            <div className="valuediv">
              <Badge
