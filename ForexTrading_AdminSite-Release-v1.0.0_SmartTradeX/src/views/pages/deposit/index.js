...
import { MoreVertical, Send, XOctagon, Edit, ChevronDown } from 'react-feather'
...
import { formatToPrice, price_to_number } from "./../../../helper/common"
import {
...
const DataTableServerSide = () => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      minWidth: '80px',
      cell: (row) => {
        const { appUserMembershipTitle } = row

        return (
          <div>
            {appUserMembershipTitle !== "" ? appUserMembershipTitle : "Thành viên mới"}
          </div>
        )
      }
    },
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      Service.send({
        method: 'POST', path: 'PaymentDepositTransaction/find', data: newParams, query: null, headers: {
  ...
  function handleApprove(item, messageSuccess) {
    Service.send({
      method: 'POST', path: 'PaymentDepositTransaction/approveDepositTransaction', data: item, query: null
    }).then(res => {
      if (res) {
...
function handleDeny(item, messageSuccess) {
  Service.send({
    method: 'POST', path: 'PaymentDepositTransaction/denyDepositTransaction', data: item, query: null
  }).then(res => {
    if (res) {
      const { statusCode, message } = res
      if (statusCode === 200) {
      ...
      >
        ...
        <ModalBody>
          ...
          <FormGroup>
            ...
            <FormGroup>
              <Label for='username'>Số tiền</Label>
              <Input              
                name='username'
                placeholder=''
            ...
            <FormGroup className='d-flex mb-0'>
              <Button.Ripple onClick={(e)=>{
                 e.preventDefault()
                 handleApprove({ id: userData.paymentDepositTransactionId,
            ...
            <Button.Ripple onClick={(e)=>{
                 e.preventDefault()
                 handleDeny({ id: userData.paymentDepositTransactionId,
