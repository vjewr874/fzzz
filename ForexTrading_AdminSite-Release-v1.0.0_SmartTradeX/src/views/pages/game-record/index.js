...
import { getGameK3Types } from "../../../constants/gameTypes";
...
function GameRecordPage({ intl }) {
  ...
  const [gameRecordType, setGameRecordType] = useState(undefined);
  ...
  const serverSideColumns = [
    ...
    // },
    {
      ...
      maxWidth: "200px",
      cell: () => {
        let currentType = getGameK3Types(intl).find(
          (item) => item.value === gameRecordType
        );
        return (
          "K3 - " + currentType?.label ||
          intl.formatMessage({ id: "minutes" }, { val: 1 })
        );
      },
    },
    {
          ...
          <>
            <div className="valuediv">
              {gameRecordValue.split("").map((item) => {
                return <div className="gameRecordValue">{item}</div>;
              })}
            </div>

            <div className="display-5 font-weight-bold mr-1">{sum}</div>
            <div className="valuediv">
              <Badge color={sum < 10 ? "danger" : "success"}>
                {intl.formatMessage({ id: sum <= 10 ? "small" : "big" })}
              </Badge>
            </div>
            <div className="valuediv">
              <Badge color={sum % 2 === 0 ? "info" : "warning"}>
                {intl.formatMessage({ id: sum % 2 === 0 ? "even" : "odd" })}
              </Badge>
            </div>
          </>
        );
    ...
    //   cell: (row) => {
    //     const { gameRecordValue } = row;
    //     const index1 = gameRecordValue?.slice(0, 1)
    //     const index2 = gameRecordValue?.slice(1, 2)
    //     const index3 = gameRecordValue?.slice(2, 3)
    //     return (<div>
    //       {parseInt(index1) + parseInt(index2) + parseInt(index3)}
    //     </div>)
    //   },
    // },
    // {
    ...
    //     const index2 = gameRecordValue?.slice(1, 2)
    //     const index3 = gameRecordValue?.slice(2, 3)
    //     const total = parseInt(index1) + parseInt(index2) + parseInt(index3)
    //     return <div>
    //       {
    //         total < 11 ?
    ...
    },
    {
      ...
      cell: (row) => {
        return (
          ...
          // <UncontrolledDrop down className='action-view'>
    ...
    <Fragment>
      <Card className="p-1">
        <SearchNav
          ...
          dropdownValue={getGameK3Types(intl)}
        ...
        <DataTable
          ...
          noDataComponent={
            <span className="mt-2">
              {intl.formatMessage({ id: "table_empty" })}
            </span>
          }
      ...
      {tranItem && sidebarBonus && (
        <Bonus
          item={tranItem}
          open={sidebarBonus}
          toggleSidebar={toggleBonus}
        />
      )}
      {tranItem && sidebarDecrease && (
        <Decrease
          item={tranItem}
          open={sidebarDecrease}
          toggleSidebar={toggleDecrease}
        />
      )}
      {tranItem && sidebarFactory && (
        <Factory
          item={tranItem}
          open={sidebarFactory}
          toggleSidebar={toggleFactory}
        />
      )}
      <Modal
        isOpen={modal}
