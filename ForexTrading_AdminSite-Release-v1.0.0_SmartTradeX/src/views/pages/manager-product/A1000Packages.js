...
function A1000Packages({ intl, open, toggleSidebar }) {
  const DefaultFilter = {
    filter: {
      packageType: "A1000FAC",
    },
    skip: 0,
    limit: 20,
    order: {
  ...
  return (
    ...
    >
      <ModalHeader toggle={toggleSidebar}>
        ...
        {intl.formatMessage({ id: "active" }).toLowerCase()}{" "}
        {intl.formatMessage({ id: "show_for_sale" })}
      </ModalHeader>
      <ModalBody>
        <Card className="p-1">
          <DataTable
...
export default injectIntl(memo(A1000Packages));
