...
const DataTableServerSide = () => {
  ...
  const serverSideColumns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>
            {formatToUSDPrice(receiveAmount)}
          </div>
        )
      }
    },
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      Service.send({
        method: 'POST', path: 'PaymentExchangeTransaction/viewExchangeRequests', data: newParams, query: null, headers: {
...
function handleDeny(item, messageSuccess) {
  Service.send({
    method: 'POST', path: 'PaymentExchangeTransaction/denyExchangeTransaction', data: item, query: null
  }).then(res => {
    if (res) {
      const { statusCode, message } = res
      if (statusCode === 200) {
          ...
          <FormGroup>
            ...
            <FormGroup>
              ...
              <Input              
                name='username'
                placeholder=''
                value={formatToPrice(userData.receiveAmount) || ''}
            ...
            <FormGroup className='d-flex mb-0'>
              <Button.Ripple onClick={(e)=>{
                 e.preventDefault()
                 handleApprove({ id: userData.paymentExchangeTransactionId,
            ...
            <Button.Ripple onClick={(e)=>{
                 e.preventDefault()
                 handleDeny({ id: userData.paymentExchangeTransactionId,
