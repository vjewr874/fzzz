...
const PacketTableServerSide = () => {
  ...
  const serverSideColumns = [
    ...
    // {
    //   name: 'Giá khuyến mãi',
    //   selector: 'packageDiscountPrice',
    //   sortable: true,
    //   maxWidth: '200px',
    //   cell: (row) => {
    //     const { packageDiscountPrice } = row

    //     return (
    //       <div>
    //         {packageDiscountPrice ? formatToPrice(packageDiscountPrice) : ""}
    //       </div>
    //     )
    //   }
    // },
  ...
  function getData(params, isNoLoading) {
    ...
    if (token) {
      ...
      Service.send({
        method: 'POST', path: 'PaymentServicePackage/find', data: newParams, query: null, headers: {
  ...
  function handleUpdateData(item, messageSuccess) {
      ...
      Service.send({
        method: 'POST', path: 'PaymentServicePackage/updateById', data: item, query: null
      }).then(res => {
        if (res) {
  ...
  function handleDeleteData(item, messageSuccess) {
    ...
    Service.send({
      method: 'POST', path: 'PaymentServicePackage/deleteById', data: item, query: null
    }).then(res => {
      if (res) {
        const { statusCode, message } = res
        if (statusCode === 200) {
  ...
  function handleAddPaymentData(item) {
    ...
    Service.send({
      method: 'POST', path: 'PaymentServicePackage/insert', data: item, query: null
    }).then(res => {
      if (res) {
        const { statusCode, message } = res
        if (statusCode === 200) {
      ...
      >
        ...
        <ModalBody>
          <Form onSubmit={handleSubmit((data) => {
            ...
            // data.packageDiscountPrice=+userData.packageDiscountPrice
          
            data.packagePrice= +userData.packagePrice
            ...
            if(userData.paymentServicePackageId){
              ...
              handleUpdateData({
                ...
                data: {
                  // packageDiscountPrice: `${userData.packageDiscountPrice}` || '',
            ...
            {/* <FormGroup>
              <Label for='packageDiscountPrice'>Giá đã gỉam</Label>
              <Input
                name='packageDiscountPrice'
                id='packageDiscountPrice'
                
                
                value={userData.packageDiscountPrice || ""}
